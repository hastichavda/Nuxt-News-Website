import { e as defineStore, c as useNuxtApp, d as useRuntimeConfig } from './server.mjs';
...
async function parseApiError(res) {
  ...
  try {
    ...
    const msg = (_a = json.results) == null ? void 0 : _a.message;
    if (msg) return msg;
  } catch {
  }
...
async function fetchNewsList(params) {
  ...
  try {
    ...
    const next = (_a = data.nextPage) != null ? _a : null;
    const total = (_b = data.totalResults) != null ? _b : null;
    return {
      articles,
      nextPage: next,
      totalResults: total,
...
const useNewsStore = defineStore("news", () => {
  ...
  const filteredArticles = computed(() => {
    ...
    return articles.value.filter((a) => {
      ...
      if (q) {
        const haystack = `${(_a = a.title) != null ? _a : ""} ${(_b = a.description) != null ? _b : ""}`.toLowerCase();
        if (!haystack.includes(q)) return false;
      }
      if (categoryFilter) {
        const categories = ((_c = a.category) != null ? _c : []).map((c) => c.toLowerCase());
  ...
  async function fetchListForCurrentRoute(query) {
    ...
    const cursor = page.value > 1 ? (_a = cursorByPage.value[page.value]) != null ? _a : null : null;
    const result = await fetchNewsList2({
      page: page.value,
      nextPage: cursor != null ? cursor : void 0,
  ...
  function getDetailState(id) {
    var _a, _b;
    return {
      article: (_a = articleById.value[id]) != null ? _a : null,
      loading: !!loadingDetailById.value[id],
      error: (_b = errorDetailById.value[id]) != null ? _b : null
    };
  }
...
//# sourceMappingURL=news-CgqK9_x2.mjs.map
